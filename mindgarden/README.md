...
**并发注册一致性**
- 用户名/邮箱的唯一性由数据库 UNIQUE 约束（`COLLATE NOCASE`，大小写不敏感）最终保证；预检只是快速路径
- 注册 = 账号 + 初始会话**同一事务**提交；并发撞约束时整体回滚，不留半成品账号
- 冲突方收到明确的 `409`（区分「该用户名已被使用」/「该邮箱已被注册」），不会得到通用 500
- 密码哈希在事务外计算，写锁只覆盖两条 INSERT，并发下互不长时间阻塞
...
- 登录按 IP+账号限流、注册按 IP+用户名与 IP 总量双限流，防暴力破解与批量注册
